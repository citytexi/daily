...
private class Solution12951 {
    fun solution(s: String): String = s.split(" ")
        .map {
            return@map when (it.isEmpty()) {
                true -> ""
                else -> {
                    val str = it.lowercase(Locale.getDefault())
                    val first = str.first()
                    when (first.digitToIntOrNull()) {
                        null -> str.replaceFirst(oldChar = first, newChar = first.uppercaseChar())
                        else -> str
                    }
                }
            }
