...
private class Solution132265 {
    fun solution(topping: IntArray): Int {
        var answer = 0
        val toppingMap = HashMap<Int, Int>()
        val firstSet = hashSetOf<Int>()
        val secondSet = hashSetOf<Int>()

        topping.forEach {
            toppingMap[it] = toppingMap.getOrDefault(it, 0) + 1
            firstSet.add(it)
        }
        ...
        for (it in topping) {
            ...
            if (toppingMap.getOrDefault(it, 0) <= 0) {
                firstSet.remove(it)
            }
...
private fun main() {
    val solution132265 = Solution132265()
    println(solution132265.solution(intArrayOf(1, 2, 1, 3, 1, 4, 1, 2)))
    println(solution132265.solution(intArrayOf(1, 2, 3, 1, 4)))
}