...
private class Solution131127 {
    fun solution(want: Array<String>, number: IntArray, discount: Array<String>): Int = when (want.filter { discount.contains(it) }.size) {
        want.size -> {
            var answer = 0
            for (i in discount.indices) {
                val result = want.filterIndexed { index: Int, s: String ->
                    duringTenDayDiscountObject(
                        want = s,
                        count = number[index],
                        discount = discount,
                        startIndex = i
    ...
    private fun duringTenDayDiscountObject(
        want: String,
        count: Int,
        discount: Array<String>,
        startIndex: Int
    ): Boolean {
        val endIndex = if (startIndex + 9 < discount.size - 1) {
            startIndex + 9
        } else {
            discount.size - 1
        }
...
private fun main() {
    val solution131127 = Solution131127()
    println(
        solution131127.solution(
