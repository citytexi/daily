...
private class Solution12978 {
    fun solution(N: Int, road: Array<IntArray>, k: Int): Int = when (N) {
        1 -> 1
        else -> {
            val D = IntArray(N + 1) {
                when (it) {
...
//                    println("${edge.toList()}, ${D.toList()}")
                    val otherNode = when (a) {
                        current -> b
                        else -> a
                    }

                    if (D[otherNode] > D[current] + weight) {
                        D[otherNode] = D[current] + weight
                    }
                }
                ...
                for (node in 1 until  D.size) {
                    val distance = D[node]

                    if (!T.contains(node)) {
                        if (distance >= D[current] && distance < vMinDistance) {
...
private fun main() {
    val solution12978 = Solution12978()
    println(
        solution12978.solution(
            N = 5,
            road = arrayOf(
                intArrayOf(1,2,1),
                intArrayOf(2,3,3),
