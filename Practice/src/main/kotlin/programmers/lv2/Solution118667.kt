...
private class Solution118667 {
    fun solution(queue1: IntArray, queue2: IntArray): Int {
        ...
        val deque1 = ArrayDeque<Long>()
        val deque2 = ArrayDeque<Long>()

        for (i in queue1.indices) {
            deque1.add(queue1[i].toLong())
            deque2.add(queue2[i].toLong())
        }

        var firstSum = deque1.sum()
        var secondSum = deque2.sum()

        while (answer <= queue1.size * 4) {
            when (firstSum) {
                secondSum -> return answer
                else -> {
                    answer += 1

                    if (firstSum > secondSum) {
                        val first = deque1.removeFirst()
                        deque2.add(first)
                        firstSum -= first
                        secondSum += first
                    } else {
                        val first = deque2.removeFirst()
                        deque1.add(first)
                        firstSum += first
                        secondSum -= first
                    }
