...
private class Solution72411 {
    fun solution(orders: Array<String>, course: IntArray): Array<String> {
        ...
        for (order in orders) {
            for (i in course) {
                combination(
                    total = order.toCharArray().sorted().joinToString(""),
                    part = "",
                    list = list,
                    n = i
                )
        ...
        for (i in course) {
            ...
            for (j in sortedMap.indices) {
                val (courseOrder, count) = sortedMap[j]

                if (courseOrder.length == i && count >= 2) {
                    if (currentCount < count) {
                        currentCount = count
                        answer = answer.plus(courseOrder)
...
private fun main() {
    val solution72411 = Solution72411()
    println(
        solution72411.solution(
