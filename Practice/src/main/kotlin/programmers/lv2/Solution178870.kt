...
private class Solution178870 {
    fun solution(sequence: IntArray, k: Int): IntArray {
        val answer: IntArray = intArrayOf(0, Int.MAX_VALUE)

        var sum = 0
        val deque = ArrayDeque<Pair<Int, Int>>()
        for (i in sequence.indices) {
            sum += sequence[i]
            deque.addLast(i to sequence[i])

            if (sum == k && answer[1] - answer[0] > deque.last().first - deque.first().first) {
                answer[0] = deque.first().first
                answer[1] = deque.last().first
            }

            while (sum > k && deque.isNotEmpty()) {
                sum -= deque.removeFirst().second
                if (sum == k && answer[1] - answer[0] > deque.last().first - deque.first().first) {
                    answer[0] = deque.first().first
                    answer[1] = deque.last().first
                }
...
private fun main() {
    val solution178870 = Solution178870()
    println(
        solution178870.solution(
