...
예를 들어, 영재가 5개의 상자를 실어야 하며, 택배 기사님이 알려준 순서가 기존의 컨테이너 벨트에 네 번째, 세 번째, 첫 번째, 두 번째,
다섯 번째 놓인 택배상자 순서인 경우, 영재는 우선 첫 번째, 두 번째, 세 번째 상자를 보조 컨테이너 벨트에 보관합니다.
...
private class Solution131704 {
    fun solution(order: IntArray): Int {
        var answer = 0
        val subStack = mutableListOf<Int>()

        for (item in 1 .. order.size) {
            val current = order[answer]

            when (item) {
                current -> answer += 1
                else -> answer = answer.popSubStack(target = item, subStack = subStack, order = order)
            }

            if (item == order.size) {
                answer = answer.popSubStack(subStack = subStack, order = order)
            }
        }
    ...
    private fun Int.popSubStack(target: Int = 0, subStack: MutableList<Int>, order: IntArray): Int {
        var tempThis = this

        while (subStack.isNotEmpty() && tempThis < order.size && order.getOrElse(tempThis) { 0 } == subStack.last()) {
            subStack.removeLast()
            tempThis += 1
        }
...
private fun main() {
    val solution131704 = Solution131704()
    println(solution131704.solution(intArrayOf(4, 3, 1, 2, 5)))
    println(solution131704.solution(intArrayOf(5, 4, 3, 2, 1)))
    println(solution131704.solution(intArrayOf(1, 2, 4, 3, 5)))
}