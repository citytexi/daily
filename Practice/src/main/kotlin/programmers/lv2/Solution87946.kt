...
private class Solution87946 {
    fun solution(k: Int, dungeons: Array<IntArray>): Int {
        var answer: Int = -1
        var iter = dungeons.size

        while (iter > 0 && answer == -1) {
            val permutationList = permutation(dungeons.map { Dungeon(it[0], it[1]) }.toTypedArray(), iter)

            for (i in permutationList.indices) {
                val list = permutationList[i]

                if (checkSuccessDungeons(list, k = k)) {
                    answer = iter
                }
            }
    ...
    private fun checkSuccessDungeons(
        list: List<Dungeon>,
        k: Int
    ): Boolean {
        var life = k
        ...
        for (count in list.indices) {
            if (life < list[count].request) {
                return false
            } else {
                life -= list[count].consume
            }
        }
...
private fun main() {
    val solution87946 = Solution87946()
    println(
        solution87946.solution(
            k = 80,
            dungeons = arrayOf(
                intArrayOf(80, 20),
                intArrayOf(50, 40),
