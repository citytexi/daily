...
private class Solution64064 {
    ...
    private fun dfs(depth: Int, target: Int, visited: IntArray, user_id: Array<String>, banned_id: Array<String>) {
        if (depth == target) {
        ...
        for (i in user_id.indices) {
            if (visited[i] == 0 && matchingId(user_id[i], banned_id[depth])) {
                visited[i] = 1
    ...
    private fun matchingId(user_id: String, banned_id: String): Boolean = when (user_id.length == banned_id.length) {
        true -> {
            var result = true
...
private fun main() {
    val solution64064 = Solution64064()
    println(
        solution64064.solution(
