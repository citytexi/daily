    ...
    fun solution(
        ...
        for (i in map.indices) {
            for (j in map[i].indices) {
                if (map[i][j] == 1) {
                    startPoint.add(i to j)
                }
            }
        }

        bfs(n, m, startPoint, map, Array(n) { BooleanArray(m) })
...
private fun main() {
    ...
    val solution7576 = Solution7576()
    bw.append("${solution7576.solution(n, m, map)}\n")
    bw.flush()
