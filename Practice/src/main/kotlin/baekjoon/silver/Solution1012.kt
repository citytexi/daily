...
private class Solution1012 {
    ...
    private var m = 0
    private var n = 0
    private var testCase: MutableList<MutableList<Int>> = mutableListOf()

    fun setTestCase(m: Int, n: Int, testCase: MutableList<MutableList<Int>>) {
        this.m = m
        this.n = n
...
private fun main() = with(System.`in`.bufferedReader()) {
    ...
    val solution = Solution1012()
    repeat(t) {
        val (m, n, k) = readLine().split(" ").map { it.toInt() }
        val list = MutableList(m) { MutableList(n) { 0 } }
        repeat(k) {
            val (x, y) = readLine().split(" ").map { it.toInt() }
            list[x][y] = 1
        }

        solution.setTestCase(
            m = m,
            n = n,
            testCase = list
        )
